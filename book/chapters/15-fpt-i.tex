...
A more complicated algorithm gives $\bigoh{n^{2.23k/3}}$, and the exponent still
grows with $k$.
...
\begin{figure}[ht]
\centering
\figplaceholder{4cm}{Nested regions: FPT innermost, then the W-hierarchy, then
XP; para-NP drawn as a separate region overlapping XP only outside FPT, with
\lang{Vertex Cover} in FPT, \lang{Clique} in W[1], \lang{Dominating Set} in
W[2], and \lang{Graph Coloring} in para-NP.}
...
\begin{figure}[ht]
\centering
\figplaceholder{4.5cm}{The three-vertex configuration $w - v - x$ with the
missing edge $wx$ drawn dashed, and the three graphs the algorithm recurses on:
$vw$ deleted, $vx$ deleted, $wx$ added.}
...
Take $s_1 = 01113$, $s_2 = 02223$, $s_3 = 01221$ and $d = 2$. The first
subproblem is $(01113, 2)$. It differs from $s_2$ in three positions, so it
branches into
\[ (02113,\,1), \qquad (01213,\,1), \qquad (01123,\,1). \]
The first of these is rejected without branching: its distance to $s_3$ is $4$,
and $4 > 2 + 1 = d + r$. The second branches into $(02213,0)$, $(01113,0)$,
$(01223,0)$ and $(01211,0)$, of which $(01223,0)$ is a solution. Note that
$(01113,0)$ merely undoes an earlier change, and a good implementation skips
positions that have already been fixed.
\end{example}
...
blocks of bad colours drops by at most $4$. For instance \texttt{abab} becomes
\texttt{abbb}, and two bad blocks become none. So $k$ changes cannot remove more
