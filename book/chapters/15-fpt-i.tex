...
time entirely: for $k = 20$ the exponential factor is only
$2^{20} \approx 10^{6}$, and it multiplies the input size instead of sitting in
its exponent. \Cref{thm:fpt-vc-kernel} will cut it further, to
$\bigoh{kn + 2^k k^2}$.
\end{example}
...
defined by the \emph{weft} of the Boolean circuits whose weighted
satisfiability a problem reduces to --- the largest number of unbounded-fan-in
gates on any path from an input to the output, with the total depth held to a
constant. A problem hard for $\mathrm{W}[1]$ or above is assumed not
...
is the following version of satisfiability: given a set of clauses \emph{each
containing at most two literals} and a parameter $k$, can at most $k$ variables
be set to true and all others to false so that every clause is satisfied? Drop
the restriction on the clause width and the same problem becomes
$\mathrm{W}[2]$-complete, as does \lang{Dominating Set}. All of them are in $\mathrm{XP}$, by trying every
$k$-set.
...
\begin{tikzpicture}[scale=0.66,
  ...
  % the parent instance, with the chosen edge marked
  \begin{scope}
    \foreach \n/\x/\y in {a/-1.3/1.2, b/-1.3/-1.2, c/2.7/1.2, d/2.7/-1.2}
      {\node[vtx] (\n) at (\x,\y) {};}
    \node[vtx,label=below:$v$] (v) at (0,0) {};
    \node[vtx,label=below:$w$] (w) at (1.4,0) {};
    \draw (v)--(a) (v)--(b) (a)--(b) (w)--(c) (w)--(d) (c)--(d);
    \draw[red!75!black,line width=1.1pt] (v)--(w);
    \node[font=\small] at (0.7,2.1) {$(G,k)$};
  \end{scope}
  % child: v deleted
  \begin{scope}[xshift=-100pt,yshift=-135pt]
    \foreach \n/\x/\y in {a/-1.3/1.2, b/-1.3/-1.2, c/2.7/1.2, d/2.7/-1.2}
      {\node[vtx] (l\n) at (\x,\y) {};}
    \node[gone] (lv) at (0,0) {};
    \node[vtx] (lw) at (1.4,0) {};
    \draw[ghost] (lv)--(la) (lv)--(lb) (lv)--(lw);
    \draw (la)--(lb) (lw)--(lc) (lw)--(ld) (lc)--(ld);
    \node[font=\small] at (0.7,-2.3) {$(G - v,\ k-1)$};
  \end{scope}
  % child: w deleted
  \begin{scope}[xshift=100pt,yshift=-135pt]
    \foreach \n/\x/\y in {a/-1.3/1.2, b/-1.3/-1.2, c/2.7/1.2, d/2.7/-1.2}
      {\node[vtx] (r\n) at (\x,\y) {};}
    \node[vtx] (rv) at (0,0) {};
    \node[gone] (rw) at (1.4,0) {};
    \draw[ghost] (rw)--(rc) (rw)--(rd) (rv)--(rw);
    \draw (rv)--(ra) (rv)--(rb) (ra)--(rb) (rc)--(rd);
    \node[font=\small] at (0.7,-2.3) {$(G - w,\ k-1)$};
  \end{scope}
  \draw[-{Latex[length=2mm]}] (0.1,-1.9) -- (-1.5,-3.3)
        node[midway,left=1pt] {$v$ in the cover};
  \draw[-{Latex[length=2mm]}] (1.3,-1.9) -- (2.9,-3.3)
        node[midway,right=1pt] {$w$ in the cover};
\end{tikzpicture}
...
subproblem is $(02231, 2)$. The candidate is at distance $0$, $3$ and $3$ from
...
change the status of only the colour it leaves and the colour it joins; a case
check shows the number of
