...
%%
%% Written against slides 1-29 of "Approximation Algorithms 2026.pdf"
%% (Johan van Rooij).  Pool question 13 (asymmetric TSP) is worked in the last
...
\begin{goals}
  \poolgoal{25.1}{define the approximation ratio for minimisation and for
        maximisation problems, and define the class APX}
  \poolgoal{25.2}{give a $2$-approximation algorithm for \lang{Max SAT} and
        prove its ratio}
  \poolgoal{25.3}{give the $2$-approximation and Christofides' algorithm for
        metric \lang{TSP}, and prove their ratios}
  \poolgoal{25.4}{describe the general shape of an approximation ratio proof: a
        quantity that bounds both $\OPT$ and the value the algorithm returns}
  \poolgoal{25.5}{give a $2$-approximation algorithm for \lang{Vertex Cover} and
        prove its ratio}
  \poolgoal{25.6}{formulate the LP relaxation of \lang{Minimum Weight Vertex
        Cover}, give the rounding algorithm, and prove that it is a
        $2$-approximation}
  \poolgoal{25.7}{explain the gap technique, and use it to show that \lang{TSP}
        without the triangle inequality admits no $c$-approximation for any
        constant $c$ unless $\mathrm{P} = \mathrm{NP}$}
  \poolgoal{13}{give a polynomial-time $c$-approximation algorithm for
...
Eulerian, and \cref{ch:stable-marriage} supplies the Euler tour. Third, the
...
loss. This is pool question~13, and it is worth working through because the
