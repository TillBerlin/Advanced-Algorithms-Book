...
%% reference).  All figures are drawn in TikZ.  fig:planar-gadget is the
%% crossover gadget of Garey-Johnson-Stockmeyer; its edge list was checked by
%% brute force over all 3^13 assignments (12 proper colourings, two orbits,
%% both required properties) and is recorded in OVERVIEW.md.
...
non-planarity. A small example of what minors can express:
...
$y$. Then $G'$ is a minor of $G$: first delete the three edges from $v$ to its
neighbours other than $x$ and $y$, and then contract the two remaining edges
$\{v,x\}$ and $\{v,y\}$, which merges $x$, $v$ and $y$ into a single vertex.
(The order matters. After the contraction, the edge from the merged vertex to a
former neighbour of $v$ may equally have come from $x$, and deleting it would
remove more than we want.) By
...
\begin{remark}[Segments with three attachments in common]%
\label{rem:planar-threeattach}
...
standard in the literature; the exercises at the end of the chapter ask you to
build a pair of segments that shows it is needed.
\end{remark}
...
linear), build the interlacement graph (at most $\bigoh{n^2}$: comparing two segments
costs time proportional to the lengths of their two attachment lists, all the
attachment lists together have total length $\bigoh{m} = \bigoh{n}$, and each
of the $\bigoh{n}$ segments is compared against all the others), and $2$-colour it (linear in its size, $\bigoh{n^2}$). So a call
costs $\bigoh{n^2}$.
...
\citet{GareyJohnsonStockmeyer1976}, with one colouring from each of its two
classes.
...
becomes a chain of $\ell$ gadgets: the vertex $u$ \emph{is} the corner $a$ of
the first gadget, the corner $a'$ of each gadget \emph{is} the corner $a$ of the
next, and only the last corner $a'$ is joined to $v$ by a real edge. The crossing pair of wires
