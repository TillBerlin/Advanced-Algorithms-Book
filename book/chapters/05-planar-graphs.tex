...
another way it is visibly plane. Establishing that a graph is \emph{not} planar
...
only witnesses its own clumsiness. In the right drawing the faces are shaded:
$f_1$, $f_2$, $f_3$ are the interior faces and $f_4$ is the outer face. Nothing
in the left drawing corresponds to them; faces exist only once an embedding is
fixed.}
\label{fig:planar-vs-plane}
\todo{The two drawings are still not identically labelled: the left one omits
the edge $\{1,6\}$, the right one omits $\{3,4\}$. They are isomorphic, but the
labels are visible, so a reader who checks will find they do not match.
Swapping $1 \leftrightarrow 3$ and $4 \leftrightarrow 6$ in the right-hand
drawing fixes it, after which the caption can say ``one graph, drawn twice''.}
\end{figure}
