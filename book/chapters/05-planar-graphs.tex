...
\begin{goals}
  \poolgoal{5.1}{give and prove Euler's formula}
  \poolgoal{5.2}{show that $m \le 3n-6$ and that the average degree is below $6$}
  \poolgoal{5.3}{define graph minors and state Kuratowski's theorem}
  \poolgoal{5.4}{show that planar graphs are $5$-colourable}
  \poolgoal{5.5}{show that $K_{3,3}$ is non-planar in an elementary way}
  \goalitem{explain how planarity can be tested in $\bigoh{n^3}$ time using segments and interlacement}
  \poolgoal{5.6}{state the planar separator theorem}
  \poolgoal{5.7}{show that \MIS{} can be solved in $2^{\bigoh{\sqrt n}}$ time on planar graphs}
  \poolgoal{5.8}{show that $3$-colouring is NP-hard for planar graphs}
\end{goals}
