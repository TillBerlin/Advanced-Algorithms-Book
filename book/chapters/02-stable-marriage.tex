...
%% Status: the stable marriage half is drafted; the Euler tour half is not.
%%
...
\begin{goals}
  \poolgoal{define an Euler tour and show that every connected graph with all even degrees must have an Euler tour}
...
\todo{Write this section: the theme that ties the chapter together. An
existence theorem can be proved by giving a procedure and showing it halts with
what was wanted; the two running examples are Euler tours below and
Gale--Shapley in \cref{sec:sm-gs}.}
...
\todo{Write this section. Definition of an Euler tour; the characterisation
that a connected graph has one if and only if every vertex has even degree;
Hierholzer's algorithm as the constructive proof of the hard direction. This is
the first item of the pool question \emph{Stable Matchings} and is not yet
covered anywhere in the book.}
