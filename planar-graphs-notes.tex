%==============================================================================
%  Lecture Notes: Planar Graphs
%  Advanced Algorithms 2026
%
%  These notes accompany the slide deck "Planar Graphs" (Planarity Testing,
%  Planar Separator Theorem + Applications) from the course archive
%  archive/2024-2025/planar.pdf.
%
%  ---------------------------------------------------------------------------
%  FIGURES
%  ---------------------------------------------------------------------------
%  This file contains NO images.  Every place where a picture would help is
%  marked with \figplaceholder{<height>}{<description>} inside a normal figure
%  environment, so the document compiles as-is and prints a framed grey box
%  describing what should be drawn.  Replace by \includegraphics later and
%  keep the surrounding \caption / \label, so cross-references keep working.
%
%  There are 23 placeholders.  This is a very visual lecture: several of the
%  arguments (segments, interlacement, the crossover gadget) are essentially
%  unreadable without a picture, and those are marked IMPORTANT in the
%  placeholder text.
%
%  ---------------------------------------------------------------------------
%  RECONSTRUCTED MATH
%  ---------------------------------------------------------------------------
%  The source PDF is password-protected; the text was recovered with
%  pdftotext, which dropped every mathematical symbol.  Formulas have been
%  reconstructed from context.  Every place where the reconstruction is a
%  judgement call is flagged with a  %% CHECK AGAINST SLIDE  comment.
%==============================================================================
...
\newcommand{\pl}{\textsc{Planar}}
\DeclareMathOperator{\dg}{deg}
\newcommand{\ThreeCol}{\textsc{3-Coloring}}
\newcommand{\PlanarThreeCol}{\textsc{Planar-3-Coloring}}
\newcommand{\MIS}{\textsc{Max-Independent-Set}}
...
\title{\bfseries Planar Graphs\\[0.4em]
       \large Lecture Notes -- Advanced Algorithms 2026}
\author{}
\date{}
...
\small
These notes follow the lecture slides \emph{Planar Graphs: Planarity Testing,
Planar Separator Theorem $+$ Applications}. They cover the same material with
the arguments written out in full, together with the informal reasoning that
usually gets spoken rather than projected. Sections marked
\emph{(non-examinable)} go beyond what was presented.
\end{minipage}
\end{center}
\vspace{1em}
...
There are two separate reasons to care, and this lecture is organised around
them.
...
\begin{itemize}[nosep]
  \item Section~\ref{sec:basics}: Euler's formula and its consequences, minors,
        Kuratowski--Wagner, colouring, Fáry's theorem.
  \item Section~\ref{sec:testing}: an $\bigoh{n^3}$ planarity test.
  \item Section~\ref{sec:pst}: the Planar Separator Theorem.
  \item Section~\ref{sec:mis}: independent set in $2^{\bigoh{\sqrt n}}$ time.
  \item Section~\ref{sec:hardness}: what remains NP-hard, and what does not.
\end{itemize}
...
A graph is \emph{planar} if it \emph{can} be drawn in $\RR^2$ with no two edges
...
\begin{figure}[ht]
\centering
\figplaceholder{45mm}{Left: a graph drawn with crossings. Right: the same graph
redrawn without crossings, with the faces shaded and labelled $f_1,\dots,f_k$,
and the outer face marked. The point of the picture is that the left drawing
does not prove the graph is non-planar --- it only proves that \emph{this
drawing} is bad.}
\caption{Planar is a property of the graph; the faces belong to the drawing.}
...
\begin{figure}[ht]
\centering
\figplaceholder{35mm}{A small plane graph containing a bridge. Highlight the
bridge and shade the single face that touches it on both sides, next to a
normal edge with its two distinct faces shaded in different colours.}
\caption{An edge has two sides; a bridge has the same face on both of them.}
\label{fig:bridge}
\end{figure}
...
\begin{figure}[ht]
\centering
\figplaceholder{45mm}{The two inductive steps of Euler's formula, side by side.
Left: a plane graph with a cycle edge $e$ highlighted, and an arrow to the same
graph with $e$ deleted, showing the two shaded faces merging into one shaded
face ($m$ and $f$ both drop by one). Right: a graph with a degree-1 vertex $v$
highlighted, and an arrow to the graph with $v$ deleted ($n$ and $m$ both drop
by one, $f$ unchanged).}
\caption{Deleting a cycle edge merges two faces; deleting a pendant vertex
changes no face.}
\label{fig:euler-induction}
\end{figure}
...
\begin{corollary}[$K_5$ is not planar]\label{cor:k5}
...
Section~\ref{sec:k33warmup}.

\begin{corollary}[Sparse graphs have sparse spots]\label{cor:deg5}
...
\begin{intuition}
Corollary~\ref{cor:deg5} is small but it is the workhorse of the whole
...
\subsection{Minors and the Kuratowski--Wagner theorem}

Corollary~\ref{cor:k5} suggests a plan: characterise planarity by forbidden
...
\begin{figure}[ht]
\centering
\figplaceholder{50mm}{IMPORTANT. Reproduce the edge-contraction example from
the slides. Left: a graph on vertices $a,b,c,d,e,f,g,h,i$ with the edge
$\{d,e\}$ highlighted. Right: the result of contracting it, with $d$ and $e$
merged into a single vertex labelled $de$ inheriting all their edges. Keep the
same layout on both sides so the eye can follow what moved.}
\caption{Edge contraction: $\{d,e\}$ is contracted to a single vertex $de$.}
...
\begin{observation}\label{obs:minor-closed}
...
\begin{theorem}[Kuratowski, Wagner]\label{thm:kuratowski}
...
$G$ is planar then by Observation~\ref{obs:minor-closed} all its minors are
planar, and $K_5$ (Corollary~\ref{cor:k5}) and $K_{3,3}$ (the exercise above)
...
``planar or not?'' graphs from the slides. These are meant to be worked on
...
\begin{remark}[Why this does not give an algorithm]\label{rem:no-algo}
Theorem~\ref{thm:kuratowski} is a characterisation, not a procedure. To use it
...
Section~\ref{sec:testing} builds a completely different algorithm. (It is
...
\begin{proposition}\label{prop:6col}
Every planar graph is $6$-colourable.
\end{proposition}

\begin{proof}
Induction on $|V|$. Graphs with at most $6$ vertices are trivially
$6$-colourable. Otherwise, by Corollary~\ref{cor:deg5} there is a vertex $v$
...
(Corollary~\ref{cor:deg5}).

\emph{Case 1: $\dg(v) \le 4$.} Exactly as in
Proposition~\ref{prop:6col}: $5$-colour $G - v$ by induction, and give $v$ one
...
Corollary~\ref{cor:k5}. Fix two non-adjacent neighbours $x$ and $y$.
...
Observation~\ref{obs:minor-closed}, $G'$ is planar. It also has $n - 2 < n$
...
\begin{figure}[ht]
\centering
\figplaceholder{55mm}{IMPORTANT. The five-colour argument in three panels.
Panel 1: vertex $v$ of degree exactly 5 with its five neighbours drawn around
it, two of them $x$ and $y$ marked as non-adjacent (dashed ``no edge'' between
them). Panel 2: $G'$, with $v$ gone and $x,y$ merged into one vertex, drawn so
that planarity is visibly preserved. Panel 3: the colouring pulled back --- $x$
and $y$ carry the same colour, the five neighbours of $v$ show only four
distinct colours, and $v$ takes the leftover colour.}
\caption{Identifying two non-adjacent neighbours frees up a colour for $v$.}
\label{fig:5col}
\end{figure}
...
from Section~\ref{sec:basics}, which is why that section came first.
\end{intuition}
...
we prove that in Section~\ref{sec:3col}. So the complexity landscape for
planar colouring is completely mapped: $k \le 2$ easy, $k = 3$ NP-hard,
$k \ge 4$ always yes.

\subsection{Drawing with straight lines}
...
\begin{figure}[ht]
\centering
\figplaceholder{40mm}{The same planar graph drawn twice: once with curved
edges, once with straight-line edges on a small integer grid (Fáry). Optionally
a third panel showing a bad straight-line drawing with huge coordinates, to
motivate the question of how small a grid suffices.}
\caption{Fáry's theorem: curves are never necessary.}
\label{fig:fary}
\end{figure}

%==============================================================================
\section{Planarity testing}\label{sec:testing}
%==============================================================================
...
Sections~\ref{sec:segments}--\ref{sec:sepcyclesec} feels abstract.
...
exactly Definition~\ref{def:interlace} and Observation~\ref{obs:2col} below,
...
Corollary~\ref{cor:3n-6}, if $m > 3n - 6$ we may immediately answer NO. So we
...
\begin{observation}[The key insight]\label{obs:key}
...
\subsection{Segments and interlacement}\label{sec:segments}
...
By Observation~\ref{obs:key}, each segment must be drawn entirely inside $C$ or
...
Reproduce the three-panel slide. Panel 1 ``Segments'': a cycle $C$ drawn as a
...
\begin{definition}[Conflict / interlacement]\label{def:interlace}
...
\begin{remark}[A case the slide glosses over]\label{rem:threeattach}
%% CHECK AGAINST SLIDE: the slide states only the interleaving condition.
...
Definition~\ref{def:interlace} (the four witnesses would have to repeat a
...
Remark~\ref{rem:threeattach}.}
\caption{Interlacing segments cannot share a side.}
\label{fig:interlace}
\end{figure}

\begin{observation}\label{obs:2col}
...
\begin{itemize}
  \item \textbf{Soundness.} If $I$ is not $2$-colourable then $G$ is not planar
        by Observation~\ref{obs:2col}. If some $C + S_i$ is not planar, then
...
\subsection{The catch, and separating cycles}\label{sec:sepcyclesec}
...
\begin{lemma}[Existence of a separating cycle]\label{lem:sepcycle}
...
\begin{figure}[ht]
\centering
\figplaceholder{60mm}{IMPORTANT. Reproduce and extend the slide picture for
Lemma~\ref{lem:sepcycle}. Draw the cycle $C$ as a circle with the two
...
\begin{algorithmic}[1]
  ...
  \If{$G$ is a cycle, or a cycle plus a path segment}
    \State \Return YES
  \EndIf
  \State find a \emph{separating} cycle $C$ (Lemma~\ref{lem:sepcycle}); let
...
details are omitted here as in the lecture.
\end{quote}
...
conversion happens in two moves. Observation~\ref{obs:key} says the only choice
is binary (inside or outside). Definition~\ref{def:interlace} records which
...
%==============================================================================
\section{The Planar Separator Theorem}\label{sec:pst}
%==============================================================================
...
\begin{theorem}[Planar Separator Theorem]\label{thm:pst}
...
%% CHECK AGAINST SLIDE: the constants 9/10 and 11 are as stated on the slide.
\end{theorem}
...
\begin{proposition}\label{prop:gridtight}
The $\sqrt{n} \times \sqrt{n}$ grid has no $\tfrac{9}{10}$-balanced separator
of size $\sqrt{n}/20$.
%% CHECK AGAINST SLIDE: constant 1/20 recovered from context.
\end{proposition}

So Theorem~\ref{thm:pst} is optimal up to the constant factor, and no cleverer
...
\begin{theorem}[Koebe's disk embedding theorem]\label{thm:koebe}
...
\begin{proof}[Sketch, for Theorem~\ref{thm:pst}]
This was omitted in the lecture and is optional reading.
Fix a disk representation of $G$ as given by Theorem~\ref{thm:koebe}.
...
Observation~\ref{obs:key}, now applied to a circle drawn through the packing.
...
%==============================================================================
\section{Application: independent set in \texorpdfstring{$2^{\bigoh{\sqrt n}}$}{2^O(sqrt n)} time}\label{sec:mis}
%==============================================================================
...
\begin{algorithmic}[1]
  ...
  \State find a $\tfrac{9}{10}$-balanced separator $S$ with
         $|S| \le 11\sqrt{n}$ \Comment{Theorem~\ref{thm:pst}}
...
Theorem~\ref{thm:pst}. Note also that this is not merely a speed-up but a
...
%==============================================================================
\section{What stays hard on planar graphs}\label{sec:hardness}
%==============================================================================

Given Section~\ref{sec:mis}, it is natural to hope that planarity tames NP-hard
...
\begin{quote}
\textsc{Planar Clique.} \emph{Given:} a planar graph $G$ and an integer $k$.
\emph{Asked:} does $G$ contain a clique of size at least $k$?
\end{quote}
...
impossible in a planar graph by Corollary~\ref{cor:k5} and
Observation~\ref{obs:minor-closed}. So every planar graph has clique number at
...
\subsection{Planar 3-Colouring is NP-hard}\label{sec:3col}

Now the other direction, as a sampler of how planar hardness proofs go.

\begin{quote}
\textsc{(Planar) 3-Coloring.} \emph{Given:} a (planar) graph $G$.
\emph{Asked:} does $G$ have a $3$-colouring?
\end{quote}
...
\begin{claim}[The crossover gadget]\label{claim:gadget}
...
picture. Reproduce the gadget $W$ exactly as drawn on the slides: the planar
...
$3$-colourings referred to in Claim~\ref{claim:gadget}: (i) all four corners
...
\begin{figure}[ht]
\centering
\figplaceholder{55mm}{Two panels, following the slides. Left: a single crossing
...
\begin{exercise}
Decide whether each of the two graphs in Figure~\ref{fig:quiz} is planar. For
...
\begin{exercise}
In the proof of Theorem~\ref{thm:5col}, where exactly do we use that $x$ and
...
Definition~\ref{def:interlace} if and only if their attachment pairs
interleave around $C$. Then construct an instance showing that
Remark~\ref{rem:threeattach} is really needed: two segments that do not
...
Lemma~\ref{lem:sepcycle}) is always planar, and that it has no separating
...
\begin{exercise}
Adapt Algorithm~\ref{alg:mis} to compute a maximum \emph{weight} independent
...
\begin{exercise}
Verify Claim~\ref{claim:gadget} on the gadget of Figure~\ref{fig:gadget} by
...
\newblock \emph{Theoretical Computer Science}, 1(3):237--267, 1976.

\bibitem{AppelHaken1977}
K.~Appel and W.~Haken.
\newblock Every planar map is four colorable.
\newblock \emph{Illinois Journal of Mathematics}, 21(3):429--567, 1977.

\bibitem{Diestel}
R.~Diestel.
\newblock \emph{Graph Theory}.
\newblock Springer, 5th edition, 2017.
\newblock (Chapter 4 for planarity, including full proofs of Kuratowski and
Fáry.)
