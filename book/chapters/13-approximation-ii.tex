...
%%
%% Written against slides 30-74 of "Approximation Algorithms 2026.pdf"
%% (Johan van Rooij).  Pool question 14 (Steiner tree) is worked in the last
...
\begin{goals}
  \poolgoal{26.1}{name the five qualities of polynomial-time approximation and
        give an example of each}
  \poolgoal{26.2}{give a problem approximable within an absolute constant
        difference, and show that maximum independent set is not one unless
        $\mathrm{P} = \mathrm{NP}$}
  \poolgoal{26.3}{define PTAS and FPTAS, and give an FPTAS for \lang{Knapsack}:
        the dynamic program, the scaling, the choice of the scaling constant,
        and the proofs of ratio and running time}
  \poolgoal{26.4}{use the gap technique to show that \lang{Knapsack} admits no
        absolute constant difference approximation unless
        $\mathrm{P} = \mathrm{NP}$}
  \poolgoal{26.5}{give the PTAS for maximum independent set on planar graphs,
        and prove its ratio and running time}
  \poolgoal{26.6}{explain the polynomial-bound technique, and use it to show
        that maximum independent set admits no FPTAS unless
        $\mathrm{P} = \mathrm{NP}$}
  \poolgoal{26.7}{give the greedy $(\ln n + 1)$-approximation for
        \lang{Set Cover} and prove its ratio}
  \poolgoal{26.8}{give the self-reduction from $G$ to $G \times G$ and use it to
        show that maximum independent set is not in APX unless it has a PTAS}
  \poolgoal{14.1}{prove that the shortest-path-metric MST heuristic is a
        $2$-approximation for \lang{Steiner Tree}}
  \poolgoal{14.2}{prove that there is no FPTAS for \lang{Steiner Tree} unless
        $\mathrm{P} = \mathrm{NP}$}
\end{goals}
...
The last section applies both halves of the chapter to one problem. This is
pool question~14.
