...
%% Status: drafted from the lecture notes; figures are still placeholders.
%%
%% Integrated from planar-graphs-notes.tex (kept in material/notes/ for
%% reference).  Two things are still open:
%%   - 23 figures are \figplaceholder boxes.  Drop the artwork into
%%     book/figures/ and replace the placeholder line, keeping the surrounding
%%     \caption and \label.  See book/figures/README.md.
%%   - Lines flagged %% CHECK AGAINST SLIDE are reconstructions of formulas
%%     that pdftotext dropped; they need checking against the original deck.
...
\begin{remark}[A case the slide glosses over]\label{rem:planar-threeattach}
...
This material was not covered in the lecture and is optional reading.
...
\subsection{From independent set to \texorpdfstring{$3$-colouring}{3-colouring}}
\label{sec:planar-3col-subexp}

\todo{Write this subsection. The lecture notes develop the separator technique
on \MIS; the exam pool (question~5, item~7) asks for the same bound for
$\ThreeCol$, so the variant has to be written out explicitly.

The template of \cref{thm:planar-mis} carries over almost verbatim. The only
change is what we guess on the separator: instead of guessing which vertices of
$S$ enter the solution, guess the colour of each vertex of $S$, which costs
$3^{|S|} = 2^{\bigoh{\sqrt n}}$ instead of $2^{|S|}$. Each component of
$G - S$ is then a $3$-colouring instance in which some vertices carry
pre-coloured neighbours, so the recursive calls must solve list-colouring
instances rather than plain $3$-colouring; state that generalisation before
recursing. Note that the subproblems are no longer independent in the naive
sense --- they share the fixed colouring of $S$ --- but for a \emph{fixed}
guess they are, which is all the recursion needs. The recurrence and the
$n \le 2^{c\sqrt n}$ bookkeeping at the end of \cref{sec:planar-mis} are
...
\begin{itemize}[leftmargin=*]
  \item J.~Hopcroft and R.~E.~Tarjan. Efficient planarity testing. \emph{Journal of the ACM}, 21(4):549--568, 1974.
  \item R.~J.~Lipton and R.~E.~Tarjan. A separator theorem for planar graphs. \emph{SIAM Journal on Applied Mathematics}, 36(2):177--189, 1979.
  \item G.~L.~Miller, S.-H.~Teng, W.~Thurston and S.~A.~Vavasis. Separators for sphere-packings and nearest neighbor graphs. \emph{Journal of the ACM}, 44(1):1--29, 1997.
  \item M.~R.~Garey, D.~S.~Johnson and L.~Stockmeyer. Some simplified NP-complete graph problems. \emph{Theoretical Computer Science}, 1(3):237--267, 1976.
  \item K.~Appel and W.~Haken. Every planar map is four colorable. \emph{Illinois Journal of Mathematics}, 21(3):429--567, 1977.
  \item R.~Diestel. \emph{Graph Theory}. Springer, 5th edition, 2017. (Chapter 4 for planarity, including full proofs of Kuratowski and Fáry.)
\end{itemize}
...
\begin{poolquestions}
  ...
  \cref{sec:planar-pst}; item~7 ($3$-colouring in $2^{\bigoh{\sqrt n}}$
  time) is \cref{sec:planar-3col-subexp}; item~8 (NP-hardness of planar
