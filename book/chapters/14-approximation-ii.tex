...
\begin{algorithmic}[1]
  ...
  \State $P \gets \max_i v(i)$ over the items that remain
...
\begin{tikzpicture}[scale=0.78,
    ...
    gone/.style={circle,draw=black!40,dashed,thin,fill=white,inner sep=1.9pt},
    ghost/.style={black!30,dotted,thick},
    every node/.style={font=\footnotesize}]
  \foreach \s/\dx in {a/0, b/6.4}{
    \begin{scope}[xshift=\dx cm]
      \foreach \l/\r in {1/2.2, 2/1.5, 3/0.8}{
        \foreach \k in {0,...,5}{
          \node[vtx] (\s\l\k) at ({\k*60}:\r) {};
        }
      }
      \node[vtx] (\s4) at (0,0) {};
    \end{scope}
  }
  % ---- panel a: the embedding with its four levels ----
  \foreach \l in {1,2,3}{
    \foreach \k [evaluate=\k as \kk using {int(mod(\k+1,6))}] in {0,...,5}{
      \draw (a\l\k)--(a\l\kk);
    }
  }
  \foreach \k in {0,...,5}{
    \draw (a1\k)--(a2\k) (a2\k)--(a3\k) (a3\k)--(a4);
  }
  \foreach \l/\r in {1/2.2, 2/1.5, 3/0.8}{
    \node[fill=white,inner sep=1pt] at ({150}:\r) {$\l$};
  }
  \node[fill=white,inner sep=1pt] at (0.34,-0.3) {$4$};
  \node at (0,-2.9) {four levels};
  % ---- panel b: level 3 deleted ----
  \begin{scope}[xshift=6.4cm]
    ...
    \foreach \k [evaluate=\k as \kk using {int(mod(\k+1,6))}] in {0,...,5}{
      \draw[ghost] (b3\k)--(b3\kk);
    }
    \foreach \k in {0,...,5}{
      \draw (b1\k)--(b2\k);
      \draw[ghost] (b2\k)--(b3\k) (b3\k)--(b4);
      \node[gone] at ({\k*60}:0.8) {};
    }
    \node at (0,-2.9) {level $3$ deleted: two pieces};
  \end{scope}
\end{tikzpicture}
...
\begin{proof}
Run the FPTAS with $\varepsilon = 1/(p(n)+1)$. Its running time is polynomial
in $n$ and in $1/\varepsilon = p(n)+1$, hence polynomial in $n$.

For a maximisation problem the guarantee gives
$\OPT/\mathrm{ALG} \le 1 + 1/(p(n)+1)$, so
\[ \mathrm{ALG} \;\ge\; \OPT \cdot \frac{p(n)+1}{p(n)+2}
   \;=\; \OPT - \frac{\OPT}{p(n)+2} \;>\; \OPT - 1, \]
using $\OPT \le p(n)$ in the last step. Since solution values are integers and
$\mathrm{ALG} \le \OPT$, this forces $\mathrm{ALG} = \OPT$. For a minimisation
problem the same choice gives
$\mathrm{ALG} \le \OPT + \OPT/(p(n)+1) < \OPT + 1$, and since
$\mathrm{ALG} \ge \OPT$ the conclusion is the same. The strict inequality is
why $p(n)+1$ was chosen rather than $p(n)$.
\end{proof}
