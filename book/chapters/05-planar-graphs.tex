...
\begin{tikzpicture}[scale=0.68,
  ...
  \begin{scope}[xshift=9.6cm]
    \node[sg,fill=green!25] (t1) at (0,1.1)   {$S_1$};
    \node[sg,fill=blue!22]  (t2) at (1.5,0)   {$S_2$};
    \node[sg,fill=green!25] (t3) at (0,-1.1)  {$S_3$};
    \node[sg,fill=green!25] (t4) at (3.0,0)   {$S_4$};
    \draw[thick] (t1)--(t2) (t3)--(t2) (t4)--(t2);
    \node[align=center] at (1.5,-2.8)
      {the interlacement graph,\\ $2$-coloured};
  \end{scope}
  %-------------------------------------------------- panel 3: the drawing
  \begin{scope}[xshift=17.4cm]
    \draw[black!40] (0,0) circle (1.7);
    \foreach \i in {1,...,8}{
    ...
    \node[align=center] at (0,-2.8) {blue inside, green outside:\\ a plane drawing};
  \end{scope}
\end{tikzpicture}
...
\begin{tikzpicture}[scale=0.8,
  ...
  \begin{scope}[xshift=5.6cm]
    ...
    \node[blue!60!black] at (0.75,0.55) {$S_1$};
    \draw[outs] (m2) .. controls (3.6,1.6) and (3.6,-1.2) .. (m5);
    \node[green!45!black] at (2.9,0.35) {$S_2$};
    \draw[outs] (m6) .. controls (-3.4,-1.6) and (-3.4,1.2) .. (m8);
    \node[green!45!black] at (-2.8,-0.3) {$S_3$};
...
\begin{tikzpicture}[scale=0.86,
  ...
  \node[red!75!black] at (1.75,0.15) {$P$};
  \node[align=center] at (0,-2.9)
  ...
  \begin{scope}[xshift=8.2cm]
    ...
    \node[red!75!black] at (2.35,1.0) {$C' = A + P$};
    \fill[blue!12] (110:1.9) arc (110:250:1.9) -- cycle;
    \node[blue!45!black] at (-1.15,0) {$B$};
    \node[green!45!black] at (0.25,-0.45) {rest of $S$};
    \node[align=center] at (0,-2.9)
...
\begin{tikzpicture}[scale=1.05,
  ...
  \node[black!60] at (0.42,0.86) {$D$};
  \node[red!75!black] at (1.28,1.28) {$C_r$};
...
\begin{figure}[ht]
\centering
\figplaceholder{60mm}{The one figure in this chapter that is not drawn here.
It needs the crossover gadget $W$ itself --- the $13$-vertex planar graph of
\citet[Figure~2]{GareyJohnsonStockmeyer1976}, which is also the one on the
lecture slides --- with its four corners $a,a'$ (left/right) and $b,b'$
(top/bottom) on the outer face. Copy that graph, and beside it show the two
essentially different $3$-colourings of Claim~\ref{claim:planar-gadget}: all
four corners the same colour, and $a,a'$ one colour with $b,b'$ another. A
guessed gadget is worse than none: the two properties are a finite case check,
so whatever is drawn should be checked against them before it goes in.}
