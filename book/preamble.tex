...
\usepackage{xcolor}
\usepackage{tikz}
\usetikzlibrary{arrows.meta,positioning,calc,patterns,shapes.geometric,fit,backgrounds,decorations.pathreplacing}
...
% Two-digit chapter numbers make entries like "12.10" wider than the book
% class's default box, so the number runs into the title in the table of
% contents.  Widen the boxes.
\makeatletter
\renewcommand*\l@section{\@dottedtocline{1}{1.5em}{2.9em}}
\renewcommand*\l@subsection{\@dottedtocline{2}{4.4em}{3.6em}}
\renewcommand*\l@subsubsection{\@dottedtocline{3}{8.0em}{4.4em}}
\makeatother

% Inside \appendix, cleveref calls every sectional unit an "appendix"; but
% "section B.2" is what a reader expects for a section of appendix B.
\crefname{subappendix}{section}{sections}
\Crefname{subappendix}{Section}{Sections}
\crefname{subsubappendix}{section}{sections}
\Crefname{subsubappendix}{Section}{Sections}

% cleveref does not know our custom theorem types by itself.
...
% --- Authoring helpers ------------------------------------------------------
% The \todo macro below marks work in progress and is deliberately visible in
% the PDF.  (Written without its braces so that `make todos', which greps for
% the literal, reports a true zero when nothing is outstanding.)
...
% A chapter's learning goals.  The exam pool is exactly this list restated as
% questions, so there is one list, not two: goals that are in the pool are
% marked, and the pool itself carries no numbering.  Usage:
%   \begin{goals}
%     \goalitem{state what is assumed as background}
%     \poolgoal{define an Euler tour and show that ...}
%   \end{goals}
\newcommand{\pooltag}{{\footnotesize\upshape[pool]}}
\newcommand{\goalitem}[1]{\item #1}
\newcommand{\poolgoal}[1]{\item #1\ \pooltag}
\newenvironment{goals}
  {\section*{Learning goals}%
   \addcontentsline{toc}{section}{Learning goals}%
   After this chapter you should be able to do each of the following. The exam
   pool is this list restated as questions, so a goal marked \pooltag{} is an
   item of one of the questions of \cref{app:pool} and can be set on the exam
   word for word.
