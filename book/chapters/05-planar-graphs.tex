...
another way it is visibly plane. Establishing that a
graph is \emph{not} planar therefore needs an argument about all possible
drawings at once, which is what \cref{sec:planar-minors,sec:planar-k33warmup}
are for.

\begin{figure}[ht]
\centering
\includegraphics[width=0.62\textwidth]{planar-vs-plane}
\caption{$K_{3,3}$ with one edge removed, drawn twice. The left drawing has
crossings; the right one does not. Only the right drawing carries information
about the graph --- a drawing with crossings never witnesses non-planarity, it
only witnesses its own clumsiness.}
\label{fig:planar-vs-plane}
\todo{The two drawings are isomorphic but not identically labelled: the left
one omits the edge $\{1,6\}$, the right one omits $\{3,4\}$. Since the vertices
carry visible labels, a reader who checks will find they do not match. Swapping
the labels $1 \leftrightarrow 3$ and $4 \leftrightarrow 6$ in the right-hand
drawing makes the two agree exactly, after which the caption can go back to
saying ``one graph, drawn twice''.}
\end{figure}
...
Johan and Jesper's notes. Left: a graph on vertices $a,b,c,d,e,f,g,h,i$ with the edge
...
``planar or not?'' graphs from Johan and Jesper's notes. These are meant to be worked on
...
Reproduce the three-panel figure from Johan and Jesper's notes. Panel 1 ``Segments'': a cycle $C$ drawn as a
...
\begin{remark}[A case Johan and Jesper's notes gloss over]\label{rem:planar-threeattach}
...
\begin{figure}[ht]
\centering
\figplaceholder{60mm}{IMPORTANT. Reproduce and extend the picture in Johan and Jesper's notes for
...
picture. Reproduce the gadget $W$ exactly as drawn in Johan and Jesper's notes: the planar
...
\begin{figure}[ht]
\centering
\figplaceholder{55mm}{Two panels, following Johan and Jesper's notes. Left: a single crossing
...
The presentation in this chapter grew out of lecture material by Johan and
Jesper, to whom the choice of topics and several of the arguments --- the
segment-and-interlacement planarity test and the crossover gadget in
particular --- are due.
